	...
	useEffect(() => {
		if (lastMessage) {
			...
			if (match) {
				...
				setMessages((prev) => [...prev, { author, timestamp, message }]);
			}
			setIsSending(false);
		}
	}, [lastMessage]);
			...
			try {
				...
				const description =
					error instanceof Error
						? error.message
						: "A problem occurred while loading chat rooms";
				toast({
				...
				>
					{selectedRoom ? (
						...
						<Center h="full" flexDirection="column" p={8}>
							...
							<Heading size="md" color={subTextColor} mb={2}>
								No chat selected
							</Heading>
